    ...
    Column(verticalArrangement = Arrangement.spacedBy(OrbitTheme.spacing.md)) {
        ...
        visible.forEach { entry ->
            PlaylistEntryCard(
                ...
                enabled = !state.enqueueing,
                qualityLabel = state.quality.label,
                onToggle = { onToggle(entry.url) },
                onPlay = { onPlayEntry(entry) },
                onDownload = { onDownloadEntry(entry) },
                onQuality = { onQuality(nextQuality(state.quality)) },
            )
        }
    }
}

/** Cycles Best → 1080p → 720p → 480p → 360p → Audio → Best. */
private fun nextQuality(current: QualityPreference): QualityPreference {
    val all = QualityPreference.entries
