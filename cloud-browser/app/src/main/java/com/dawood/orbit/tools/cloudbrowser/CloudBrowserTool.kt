...
import com.dawood.orbit.tools.cloudbrowser.screens.BookmarksScreen
...
fun CloudBrowserTool(tool: Tool, onBack: () -> Unit, modifier: Modifier = Modifier) {
    ...
    val bookmarkStore = remember(context) { BookmarkStore.get(context) }
    val historyStore = remember(context) { HistoryStore.get(context) }
    val bookmarks by bookmarkStore.items.collectAsStateWithLifecycle()
    val historyEntries by historyStore.items.collectAsStateWithLifecycle()
    ...
    // URL handed in from bookmarks/history before a session existed; the
    // launch flow navigates here instead of the default home page.
    ...
    // Standalone extra: record browsing history on the phone as the remote
    // page changes. The store merges rapid repeats of the same URL.
    LaunchedEffect(page.url, page.title, route) {
        if (route == CloudRoute.BrowserView && page.url.startsWith("http")) {
            historyStore.record(page.url, page.title)
        }
    }
        ...
        scope.launch {
            ...
            if (initialNavDone.add(session.id)) {
                val first = withContext(Dispatchers.IO) {
                    api.navigate(session.id, pendingUrl ?: HOME_URL)
                }
    ...
    /** Opens [url] in the live tab, or launches a session aimed at it. */
    fun openUrl(url: String) {
        val id = activeSessionId
        if (connectionState == ConnectionState.Connected && id != null) {
    ...
    fun shareCurrentPage() {
        ...
        runCatching {
            ...
            context.startActivity(
                Intent.createChooser(send, "Share page")
    ...
    val browserActions = remember(api) {
        BrowserViewActions(
            ...
            onToggleBookmark = {
                val url = page.url
                if (!url.startsWith("http")) return@BrowserViewActions
                val title = page.title.ifBlank { BrowserLinks.hostOf(url) }
                scope.launch(Dispatchers.IO) { bookmarkStore.toggle(url, title) }
            },
    ...
    ToolShell(
        ...
        menuContent = { dismiss ->
            if (route == CloudRoute.BrowserView && page.url.startsWith("http")) {
                val bookmarked = bookmarks.any { it.url == page.url }
                OrbitMenuItem(
                    text = if (bookmarked) "Remove bookmark" else "Bookmark this page",
                    onClick = {
                        dismiss()
                        val title = page.title.ifBlank { BrowserLinks.hostOf(page.url) }
                        scope.launch(Dispatchers.IO) {
                            bookmarkStore.toggle(page.url, title)
                        }
                    },
                    icon = if (bookmarked) OrbitIcons.Bookmark else OrbitIcons.BookmarkOutline,
                )
                OrbitMenuItem(
                    text = "Share page",
                    onClick = { dismiss(); shareCurrentPage() },
                    icon = OrbitIcons.Share,
                )
                OrbitMenuItem(
                    text = "Open in external browser",
                    onClick = { dismiss(); openCurrentPageExternally() },
                    icon = OrbitIcons.OpenExternal,
                )
            }
            OrbitMenuItem(
                text = "Bookmarks & history",
                onClick = { dismiss(); push(CloudRoute.Bookmarks) },
            ...
            if (settings.introSeen && route == CloudRoute.BrowserView) {
                ...
                BrowserViewScreen(
                    state = BrowserViewState(
                        ...
                        isBookmarked = bookmarks.any { it.url == page.url },
                    ),
                        ...
                        Column(verticalArrangement = Arrangement.spacedBy(CloudSpacing.PadMd)) {
                            if (!settings.introSeen) {
                                ...
                                when (route) {
                                    ...
                                    CloudRoute.Bookmarks -> BookmarksScreen(
                                        bookmarks = bookmarks,
                                        history = historyEntries,
                                        onOpen = ::openUrl,
                                        onDeleteBookmark = { id ->
                                            scope.launch(Dispatchers.IO) { bookmarkStore.remove(id) }
                                        },
                                        onDeleteHistoryEntry = { id ->
                                            scope.launch(Dispatchers.IO) { historyStore.remove(id) }
                                        },
                                        onClearHistory = {
                                            scope.launch(Dispatchers.IO) { historyStore.clearHistory() }
                                        },
                                    )
                                }
                            }
